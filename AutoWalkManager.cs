...
public class AutoWalkManager
{
    ...
    /// <summary>
    /// Set the health-pause delegate. Called by GameManager after HealthManager is created.
    /// Separate from ShouldPauseCommands so CastCoordinator can still cast while resting.
    /// </summary>
    public void SetHealthPauseCheck(Func<bool> shouldPauseForHealth)
    {
        _shouldPauseForHealth = shouldPauseForHealth;
    }

    /// <summary>
    /// Set the party-wait pause delegate. Called by GameManager after PartyManager is created.
    /// Separate from health pause so the two systems can be independently managed.
    /// </summary>
    public void SetPartyWaitPauseCheck(Func<bool> shouldPauseForPartyWait)
    {
        _shouldPauseForPartyWait = shouldPauseForPartyWait;
    }
    ...
    private void SendNextStep()
    {
        ...
        // Check if paused for health management (resting/meditating/fleeing)
        if (_shouldPauseForHealth?.Invoke() == true)
        {
            _logMessage("⏸️ Auto-walk paused — health management active");
            SetState(AutoWalkState.Paused);
            StartPausePollTimer();
            return;
        }

        // Check if paused for party wait (leader waiting for members)
