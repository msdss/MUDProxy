...
// Message Router
    ...
    // Health Management
    private bool _healthHangupActive = false;
    private bool _pendingHangupDisconnect = false;
    ...
    private void TelnetConnection_OnStatusChanged(bool connected, string statusMessage)
    {
        ...
        {
            // Inject session-ended message into game terminal display.
            // Only for actual disconnect events — not intermediate states like
            // "Connecting..." or "Retrying in Xs..." which TelnetConnection fires
            // during reconnect attempts.
            if (_pendingHangupDisconnect)
            {
                _pendingHangupDisconnect = false;
                DisplayMudTextDirect($"\r\n\x1B[1;97;41m[EMERGENCY HANGUP - Session ended ({DateTime.Now:h:mm tt})]\x1B[0m\r\n");
                _terminalControl.InvalidateTerminal();
            }
            else if (statusMessage == "Connection lost")
            {
                DisplayMudTextDirect($"\r\n\x1B[1;97;44m[Connection lost ({DateTime.Now:h:mm tt})]\x1B[0m\r\n");
                _terminalControl.InvalidateTerminal();
            }
            else if (statusMessage == "Disconnected")
            {
                DisplayMudTextDirect($"\r\n\x1B[1;97;44m[Session ended ({DateTime.Now:h:mm tt})]\x1B[0m\r\n");
                _terminalControl.InvalidateTerminal();
            }
    ...
    private void MessageRouter_OnCombatStateChanged(bool inCombat)
    {
        ...
        // When combat ends (no more enemies), re-evaluate health thresholds
        // so auto-rest/meditate can trigger
        if (!inCombat)
            _gameManager.HealthManager.OnCombatEnded();
    }
    ...
    {
        RecordTick();
        _gameManager.AutoWalkManager.OnCombatTick();
    }
    ...
    private void InitializeBuffManagerEvents()
    {
        ...
        _gameManager.HealthManager.OnBlockReconnect += () => BeginInvoke(() => _healthHangupActive = true);
        _gameManager.HealthManager.OnAllowReconnect += () => BeginInvoke(() => _healthHangupActive = false);
        _gameManager.HealthManager.OnFleeRequested += () => BeginInvoke(HandleFleeMovement);
        _gameManager.HealthManager.OnHealthActionChanged += action => BeginInvoke(() => UpdateHealthActionDisplay(action));
    ...
    private void InitializeComponent()
    {
        ...
        optionsMenu.DropDownItems.Add(new ToolStripMenuItem("Loop Editor...", null, LoopEditor_Click) { ForeColor = Color.White, BackColor = Color.FromArgb(45, 45, 45) });
    ...
    private void HandleRemoteHangup()
    {
        ...
        _pendingHangupDisconnect = true;
        Disconnect();
    }
    
    /// <summary>
    /// Handle flee movement request from HealthManager.
    /// Sends the backtrack movement command based on current walk path.
    /// </summary>
    private void HandleFleeMovement()
    {
        var walkManager = _gameManager.AutoWalkManager;
        var steps = walkManager.Steps;
        var currentIndex = walkManager.CurrentStepIndex;

        // Try to backtrack along the path
        if (steps.Count > 0 && currentIndex > 0)
        {
            // Get the step we just completed and reverse its direction
            var lastStep = steps[currentIndex - 1];
            var reverseDir = HealthManager.GetReverseDirection(lastStep.Command);
            if (reverseDir != null)
            {
                LogMessage($"🏃 Fleeing: {reverseDir}", MessageType.System);
                SendCommandToServer(reverseDir);
                return;
            }
        }

        // Fallback: if not on a path or can't reverse, try any available exit
        var currentRoom = _gameManager.RoomTracker.CurrentRoom;
        if (currentRoom?.Exits != null && currentRoom.Exits.Count > 0)
        {
            // Pick first available normal exit
            foreach (var exit in currentRoom.Exits)
            {
                if (exit.ExitType == RoomExitType.Normal && !string.IsNullOrEmpty(exit.Direction))
                {
                    var dir = exit.Direction.ToLowerInvariant();
                    LogMessage($"🏃 Fleeing: {dir}", MessageType.System);
                    SendCommandToServer(dir);
                    return;
                }
            }
        }

        LogMessage("🏃 Fleeing: no available exit found!", MessageType.System);
    }

    /// <summary>
    /// Update UI to reflect current health management action.
    /// </summary>
    private void UpdateHealthActionDisplay(HealthAction action)
    {
        // Currently just log — can be extended to show status in UI later
    }
