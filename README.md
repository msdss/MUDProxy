...
Central coordinator (owns all 16 sub-managers):
...
Health/mana threshold automation:
- 7-priority evaluation: Hang > Run > Meditate (priority) > Rest HP > Meditate/Rest Mana > Stop Resting > Stop Meditating
- Two-threshold hysteresis: START threshold begins recovery, MAX threshold ends it
- `_restingForMana` tracks trigger reason so Priority 6 checks the correct resource
- Post-combat 250ms delay prevents rest/meditate on stale enemy data
- `ShouldPauseWalking` pauses AutoWalkManager without blocking CastCoordinator
- ALL OFF gating: `Evaluate()`, `OnCombatEnded()`, `OnRestingStateChanged()` check `_isAnyAutomationEnabled()`
- Deferred eval timer in `OnCombatEnded()` fires `Evaluate()` when delay expires
- `OnRestingStateChanged()` re-sends rest/meditate using MAX threshold when interrupted
- Emergency hangup disconnects immediately, blocks reconnect until manual
...
1. **GameManager is the central coordinator** — Owns all 16 sub-managers. Access via `_gameManager.CombatManager`, `_gameManager.RoomTracker`, etc. MainForm's `_buffManager` is a shortcut reference to `_gameManager.BuffManager`.
