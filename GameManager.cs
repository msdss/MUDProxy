...
public class GameManager
{
    ...
    private NavigationSettings _navigationSettings = new();
    private HealthSettings _healthSettings = new();
    ...
    public NavigationSettings NavigationSettings
    {
        ...
        set { _navigationSettings = value; _profileManager.HasUnsavedChanges = true; }
    }

    public HealthSettings HealthSettings
    {
        get => _healthSettings;
        set { _healthSettings = value; _profileManager.HasUnsavedChanges = true; }
    }
    ...
    public GameManager()
    {
        ...
        _partyManager = new PartyManager(
            () => ShouldPauseCommands,
            () => (_combatManager?.CombatEnabled ?? false) || (_healingManager?.HealingEnabled ?? false) ||
                  (_buffManager?.AutoRecastEnabled ?? false) || (_cureManager?.CuringEnabled ?? false),
        ...
        _autoWalkManager = new AutoWalkManager(
            ...
            () => _combatManager.ClearAlsoHereDedup(),
        ...
        _combatManager.OnCombatActivityDetected += _autoWalkManager.OnCombatTick;

        _loopManager = new LoopManager(
            _autoWalkManager,
            _roomGraphManager,
            _roomTracker,
            _playerStateManager.ExperienceTracker,
            msg => OnLogMessage?.Invoke(msg)
        );

        _loopManager.OnLogMessage += msg => OnLogMessage?.Invoke(msg);
        ...
        // Create HealthManager — threshold-based rest/meditate/flee/hangup
        _healthManager = new HealthManager(
            () => _playerStateManager.CurrentHpPercent,
            () => _playerStateManager.CurrentManaPercent,
            () => _playerStateManager.MaxHp,
            () => _playerStateManager.MaxMana,
            () => _playerStateManager.CurrentHp,
            () => _playerStateManager.CurrentMana,
            () => _playerStateManager.IsResting,
            () => _playerStateManager.IsMeditating,
            () => _playerStateManager.InCombat,
            () => _combatManager.CurrentRoomEnemies.Count,
            () => _combatManager.CombatEnabled || _healingManager.HealingEnabled ||
                  _buffManager.AutoRecastEnabled || _cureManager.CuringEnabled,
            () => _healthSettings,
            msg => OnLogMessage?.Invoke(msg)
        );
        _healthManager.OnSendCommand += cmd => OnSendCommand?.Invoke(cmd);
        _healthManager.OnHangupRequested += () => OnHangupRequested?.Invoke();
        _healthManager.OnLogMessage += msg => OnLogMessage?.Invoke(msg);

        // Wire PlayerStateManager → HealthManager (on every HP/Mana prompt, not just stat/exp)
        _playerStateManager.OnHpManaPromptProcessed += () => _healthManager.OnHpPromptReceived();
        _playerStateManager.OnRestingStateChanged += (isResting, isMeditating) =>
            _healthManager.OnRestingStateChanged(isResting, isMeditating);

        // Wire CastCoordinator → HealthManager (re-rest after buff/heal cast)
        _castCoordinator.OnCastCompleted += () => _healthManager.OnCastCompleted();

        // Wire RoomTracker → HealthManager (for intelligent flee)
        _roomTracker.OnRoomChanged += room => _healthManager.OnRoomChanged();

        // Wire health-pause into AutoWalkManager (separate from ShouldPauseCommands
        // so CastCoordinator can still cast buffs/heals while resting)
        _autoWalkManager.SetHealthPauseCheck(() => _healthManager.ShouldPauseWalking);

        // Wire HealthManager → PartyManager (follower sends @Wait/@Ok on health action changes)
        _healthManager.OnHealthActionChanged += action => _partyManager.OnHealthActionChanged(action);
    ...
    /// <summary>
    /// Called when login completes after a reconnection.
    /// Waits 5 seconds for startup commands (stat, exp, who, party invites)
    /// to process before resuming any walk or loop.
    /// </summary>
    public void OnLoginComplete()
    {
        if (_loopManager.IsActive || _autoWalkManager.State == AutoWalkState.Disconnected)
        {
            ...
            resumeTimer.Elapsed += (s, e) =>
            {
                ...
                {
                    _loopManager.OnReconnected();
                }
                else if (_autoWalkManager.State == AutoWalkState.Disconnected)
                {
    ...
    public void NewCharacterProfile()
    {
        ...
        _healthSettings = new HealthSettings();
    ...
    public (bool success, string message) LoadCharacterProfile(string filePath)
    {
        ...
        if (profile.NavigationSettings != null)
            _navigationSettings = profile.NavigationSettings.Clone();

        if (profile.HealthSettings != null)
