...
public class CombatManager
{
    ...
    private List<string> _playersInRoom = new();  // First names of players currently in room
    ...
    public event Action? OnCombatActivityDetected; // Event when combat activity (dodge/miss) is detected
    ...
    public void ProcessMessage(string message)
    {
        ...
        // Detect combat activity (dodge/miss messages) for the heartbeat safety net.
        // These simple string checks reset AutoWalkManager's heartbeat timer so it
        // doesn't false-fire during all-dodge rounds. Order: cheapest checks first.
        if (OnCombatActivityDetected != null)
        {
            if (message.Contains("but you dodge!", StringComparison.OrdinalIgnoreCase) ||
                message.Contains("but he dodges!", StringComparison.OrdinalIgnoreCase) ||
                message.Contains("but she dodges!", StringComparison.OrdinalIgnoreCase) ||
                message.Contains("at you", StringComparison.OrdinalIgnoreCase) ||
                ContainsPlayerSwing(message))
            {
                OnCombatActivityDetected.Invoke();
            }
        }
    }
    ...
    private void ProcessAlsoHere(string alsoHereContent)
    {
        ...
        // Clear previous room enemies and players
        _currentRoomEnemies.Clear();
        ...
        {
            OnPlayersDetected?.Invoke(_playersInRoom);
        }
    ...
    /// <summary>
    /// Process "Also here:" content forwarded directly from RoomTracker.
    /// Called during room detection to guarantee the enemy list is populated
    /// before OnRoomChanged fires, preventing the race condition where the
    /// walker sends the next move command before enemies are detected.
    /// </summary>
    public void ProcessAlsoHereDirect(string alsoHereContent)
    {
        if (!_combatEnabled)
            return;

        ProcessAlsoHere(alsoHereContent);
    }

    /// <summary>
    /// Check if a server message contains "at {playerName}" for any player currently
    /// in the room, or the player character themselves. Catches non-damaging swing
    /// messages targeting other players (e.g., "The Goblin swings at Azii").
    /// </summary>
    private bool ContainsPlayerSwing(string message)
    {
        // Check current player character name
        if (!string.IsNullOrEmpty(_currentCharacter) &&
            message.Contains($"at {_currentCharacter}", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }

        // Check players currently in the room (first names from last "Also here:" parse)
        foreach (var playerName in _playersInRoom)
        {
            if (message.Contains($"at {playerName}", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        }
    ...
    /// <summary>
    /// Prepare for a fresh room scan by clearing the dedup guard, enemy list,
    /// and player list. This ensures that if the server responds WITHOUT an
    /// "Also here:" line (empty room), the stale lists read as empty rather
    /// than retaining data from the previous scan.
    ///
    /// Does NOT clear _currentTarget, _attackPending, or any other combat
    /// state — preserving the "already fighting same target" guard in
    /// TryInitiateCombat() so no harmful re-attack commands are sent.
    ///
    /// Used by AutoWalkManager's combat heartbeat room check.
    /// </summary>
    public void ClearAlsoHereDedup()
    {
        _lastProcessedAlsoHere = string.Empty;
        _currentRoomEnemies.Clear();
        _playersInRoom.Clear();
