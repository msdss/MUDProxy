...
public class PlayerInfo
{
    ...
    // Combat/skill stats (parsed from stat command)
...
/// <summary>
/// Per-character navigation preferences for the auto-walker.
/// </summary>
public class NavigationSettings
{
    /// <summary>
    /// When true, the auto-walker uses pick + open to traverse doors.
    /// When false (default), the auto-walker uses bash.
    /// All characters can bash. Only characters with picklock skill should enable picking.
    /// </summary>
    public bool UsePicklockInsteadOfBash { get; set; } = false;

    /// <summary>
    /// Maximum number of bash/pick attempts before giving up. 0 = unlimited.
    /// </summary>
    public int MaxDoorAttempts { get; set; } = 10;

    /// <summary>
    /// Maximum number of search attempts for hidden exits before giving up. 0 = unlimited.
    /// </summary>
...
/// <summary>
/// Per-character health and mana/kai management settings.
/// Controls auto-rest, auto-flee, and emergency disconnect thresholds.
/// </summary>
public class HealthSettings
{
    // Health thresholds (percent 0–100, 0 = disabled)
    public int HpRestMaxPercent { get; set; } = 100;
    public int HpRestBelowPercent { get; set; } = 0;
    public int HpRunBelowPercent { get; set; } = 0;
    public int HpHangBelowPercent { get; set; } = 0;

    // Mana/Kai thresholds (percent 0–100, 0 = disabled)
    public int ManaRestMaxPercent { get; set; } = 100;
    public int ManaRestBelowPercent { get; set; } = 0;

    // Mana options
    public bool UseMeditateAbility { get; set; } = false;
    public bool MeditateBeforeResting { get; set; } = false;

    // Common options
    public bool AllowHangInAllOff { get; set; } = false;
    public bool UseIntelligentRun { get; set; } = false;
    ...
    public HealthSettings Clone()
    {
        ...
        {
            HpRestMaxPercent = this.HpRestMaxPercent,
            HpRestBelowPercent = this.HpRestBelowPercent,
            HpRunBelowPercent = this.HpRunBelowPercent,
            HpHangBelowPercent = this.HpHangBelowPercent,
            ManaRestMaxPercent = this.ManaRestMaxPercent,
            ManaRestBelowPercent = this.ManaRestBelowPercent,
            UseMeditateAbility = this.UseMeditateAbility,
            MeditateBeforeResting = this.MeditateBeforeResting,
            AllowHangInAllOff = this.AllowHangInAllOff,
            UseIntelligentRun = this.UseIntelligentRun,
...
public class CharacterProfile
{
    ...
    // Health/Mana management settings
...
public enum RoomExitType
{
    ...
    /// <summary>Searchable hidden exit — requires "sea {direction}" to reveal before traversal.</summary>
    SearchableHidden,
...
public class PathResult
{
    ...
    /// <summary>Aggregated requirements for the path (door stats, level, items).</summary>
...
public class PathStep
{
    ...
    /// <summary>The type of exit (Normal, Door, Text, etc.). Used by AutoWalkManager for special handling.</summary>
    public RoomExitType ExitType { get; set; } = RoomExitType.Normal;
    /// <summary>Picklocks/strength requirement for stat-gated doors. 0 = no requirement.</summary>
    public int DoorStatRequirement { get; set; } = 0;
...
/// <summary>
/// Aggregated requirements across all steps in a path.
/// Used by WalkToDialog to inform the user before walking.
/// </summary>
public class PathRequirements
{
    /// <summary>Highest picklocks/strength requirement across all door steps. 0 = no doors or no requirements.</summary>
    public int MaxDoorStatRequirement { get; set; } = 0;
