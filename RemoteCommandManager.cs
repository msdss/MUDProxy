...
public class RemoteCommandManager
{
    ...
    public event Action<string>? OnWaitCommandReceived;  // Party @Wait from senderName
    public event Action<string>? OnOkCommandReceived;    // Party @Ok from senderName
    ...
    private void ProcessCommand(string senderName, string commandText)
    {
        ...
        // Party coordination commands — bypass permission check (validated by PartyManager)
        if (command == "@wait")
        {
            OnLogMessage?.Invoke($"📡 @Wait received from {senderName}");
            OnWaitCommandReceived?.Invoke(senderName);
            return;
        }
        if (command == "@ok")
        {
            OnLogMessage?.Invoke($"📡 @Ok received from {senderName}");
