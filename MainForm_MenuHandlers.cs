...
public partial class MainForm
{
    ...
    private async void ConnectButton_Click(object? sender, EventArgs e)
    {
        ...
        // If reconnecting after a health hangup, clear the hangup state
        // (the safety check for enemies happens after room detection in HealthManager)
        if (_healthHangupActive)
        {
            _gameManager.HealthManager.ClearHangupState();
            _healthHangupActive = false;
            LogMessage("🔄 Manual reconnect after health hangup — monitoring for enemies", MessageType.System);
        }
